...
namespace Cainos.InteractivePixelWater
{
    ...
    public class PixelWaterEditor : Cainos.LucidEditor.LucidEditor
    {
        ...
        private void OnSceneGUI()
        {
            ...
            var fmh_53_66_639108267854943095 = Quaternion.identity; Vector3 newBL = Handles.FreeMoveHandle(handlePos[0], handleSize, snap, Handles.CubeHandleCap);
            ...
            //bottom right handle
            EditorGUI.BeginChangeCheck();
            var fmh_65_66_639108267854953361 = Quaternion.identity; Vector3 newBR = Handles.FreeMoveHandle(handlePos[1], handleSize, snap, Handles.CubeHandleCap);
            if (EditorGUI.EndChangeCheck())
            {
                Undo.RecordObject(water, "Resize Water");
                Undo.RecordObject(water.transform, "Move Water");
            ...
            //top left handle
            EditorGUI.BeginChangeCheck();
            var fmh_77_66_639108267854955667 = Quaternion.identity; Vector3 newTL = Handles.FreeMoveHandle(handlePos[2], handleSize, snap, Handles.CubeHandleCap);
            if (EditorGUI.EndChangeCheck())
            {
                Undo.RecordObject(water, "Resize Water");
                Undo.RecordObject(water.transform, "Move Water");
            ...
            //top right
            EditorGUI.BeginChangeCheck();
            var fmh_89_66_639108267854957441 = Quaternion.identity; Vector3 newTR = Handles.FreeMoveHandle(handlePos[3], handleSize, snap, Handles.CubeHandleCap);
            if (EditorGUI.EndChangeCheck())
            {
                Undo.RecordObject(water, "Resize Water");
                Undo.RecordObject(water.transform, "Move Water");
